...
def hyb_n(xyz):
    radii = pd.read_csv('/pscratch/sd/s/schandy/delta_learning/new_data/ele_cl/vdw_radii.txt')
...
def plot_scatter(x,y,plot_save_path,title):
    ...
    ax.plot(x_ticks,x_ticks,zorder=0, linestyle='dotted')
    ax.set_xlabel(r'$E_{eda}-E_{ff}$')
    ax.set_ylabel(r'$E_{delta-ml}$')
    ax.set_title(title)
    ax.grid(True)
    plt.tight_layout()
...
def plot_box(x, plot_save_path, title=r'$E_{eda}-E_{ff} \quad vs \quad E_{delta-ml}$'):
    plt.figure(figsize=(4, 3), dpi=600)
