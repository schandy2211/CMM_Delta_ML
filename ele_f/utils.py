...
def hyb_n(xyz):
    ...
    R_e = np.array([[0, 0.959273, 0.959273, 0],
                    [0.959273, 0, 1.522472, 0],
                    [0.959273, 1.522472, 0, 0],
                    [0, 0, 0, 0]])
...
def Matrix_hyb(dist, R_e, R_vdw):
    ...
    A = np.exp(-dist[3, :3] / R_vdw[3, :3]) / (dist[3, :3] + 1e-6)  
    M_hyb[3, :3] = A
    B = np.exp(-dist[:3, 3] / R_vdw[:3, 3]) / (dist[:3, 3] + 1e-6)  
    M_hyb[:3, 3] = B
