...
def radii_vdw(origin_data,radii):
    ...
    r_vdw = np.array(r_vdw_list).reshape((6,))
...
def hyb_n(xyz):
    ...
    file_path = os.path.join(direct, 'vdw_radii.txt')
    radii = pd.read_csv(file_path)
    dist_all = np.array([dist(x) for x in xyz], dtype=np.float64)
    R_vdw = radii_vdw(xyz[0], radii)
    R_vdw = np.array(R_vdw)

    R_e = np.array([
        [0, 0.959273, 0.959273, 0, 0, 0],   # O1 - H1, H2, O2, H3, H4
        [0.959273, 0, 1.522472, 0, 0, 0],   # H1 - O1, H2, O2, H3, H4
        [0.959273, 1.522472, 0, 0, 0, 0],   # H2 - O1, H1, O2, H3, H4
        [0, 0, 0, 0, 0.959273, 0.959273],   # O2 - O1, H1, H2, H3, H4
        [0, 0, 0, 0.959273, 0, 1.522472],   # H3 - O1, H1, H2, O2, H4
        [0, 0, 0, 0.959273, 1.522472, 0]    # H4 - O1, H1, H2, O2, H3
    ])  # Use actual distances for water dimer equilibrium

    # Compute hybridization matrix for each distance matrix
...
def Matrix_hyb(dist, R_e, R_vdw):
    epsilon = 1e-8
    # Initialize M_hyb with bonded terms: (dist - R_e) / dist
    M_hyb = np.array((dist - R_e) / (dist + epsilon), dtype=np.float64)

    # Set diagonal elements to 0 (no self-interaction)
    np.fill_diagonal(M_hyb, 0)

    # Non-bonded terms for specific indices (update using exponential decay)

    # O1 (index 0) interactions with H3, H4, H5 (indices 3, 4, 5)
    A = np.exp(-dist[0, 3:6] / R_vdw[0, 3:6])
    M_hyb[0, 3:6] = A
    # Similarly for O2 (index 3) interactions with H1, H2 (indices 1, 2)
    B = np.exp(-dist[3:6, 0] / R_vdw[3:6, 0])
    M_hyb[3:6, 0] = B

    # H1, H2 (indices 1, 2) interactions with H4, H5 (indices 4, 5)
    A_hh = np.exp(-dist[1:3, 4:6] / R_vdw[1:3, 4:6])
    M_hyb[1:3, 4:6] = A_hh
    B_hh = np.exp(-dist[4:6, 1:3] / R_vdw[4:6, 1:3])
    M_hyb[4:6, 1:3] = B_hh
...
def load_xyz(file_path):
    ...
    data_blocks = data.strip().split("6\n\n")
