...
def prepare_tensors(hyb_data, E_data):
    x = torch.tensor(hyb_data).view(len(hyb_data), 36).float().to(device)
    if isinstance(E_data, pd.DataFrame):
        y = torch.tensor(E_data.to_numpy()).float().to(device)  # Convert DataFrame to NumPy array
    else:
        y = torch.tensor(E_data).float().to(device)  # Directly convert if it's already a NumPy array
...
model.eval()
output = model(torch.tensor(hyb_valid).view(len(hyb_valid), 36).float().to(device)).cpu().detach().numpy()
...
mae_results_df.to_csv(save_path + 'MAE_Comparison.txt', index=False, sep=' ', float_format='%.8f')
